...
impl Partition {
    ...
    pub fn create_read_buffer_chunk(&mut self, table_name: &str, chunk: read_buffer::Chunk) -> Result<Arc<RwLock<Chunk>>> {
        let table = self
            .tables
        ...
        let chunk = Arc::new(self.metrics.new_lock(Chunk::new_read_buffer(
            chunk_id,
            &self.key,
            Arc::new(chunk),
            self.metrics.new_chunk_metrics(),
